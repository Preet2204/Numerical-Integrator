...
int main() {
    ...
    while(true) {
        ...
        // Printinf 1st Row
        integral[0] = Trapezoidal(lower_limit, upper_limit, intervals, func);
        error[0] = abs(integral[0] - actualIntegral);
        cout << left << setw(col1) << "Integral";
        cout << left << setw(col2) << fixed << setprecision(6) << integral[0];
        
        if(intervals % 2 == 0) {
            integral[1] = Simpson13(lower_limit, upper_limit, intervals, func);
        ...
        if(intervals % 3 == 0) {
            integral[2] = Simpson38(lower_limit, upper_limit, intervals, func);
